...
2. use "mcp servers" tab to add mcp servers as mcp gateway service if mcp servers added as streamablehttp server with `oauth`, you'd need to click "authorize" link for added mcp server gateway to authorize and fetch tools, please note that for `redirect URI`, UI gives a hint URL, the host part of the URL needs to be replaced with `<zen-route>`, and this URL needs to match
what's configured in your `oauth` application as callback URL  
3. use "virtual servers" tab to create a virtual server that you'd like to have all or subset of available tools included for the server, virtual server is available at `https://zen-route/physical_location/default-pl/<app_name-app_run_id>/servers/<virtual-server-id>/mcp` that mcp client can connect to
...
1. create secret called mcp-gateway-secret:  
  `oc create secret generic mcp-gateway-secret --from-literal=token='<mcp gateway api token>' -n <zen namespace>`. 
